...
function getOne(req, res){
	var profileType = {};
	ProfileType.findOne({ name: req.params.name }, function(err, profileType){
		Profile.findOneAndUpdate({ name: req.params.name , locked: false }, { locked: true }, {}, function(err, result){
...
function releaseOne(req, res){
	Profile.findByIdAndUpdate(req.params.id, { locked: false }, {}, function(err, result){
       res.json(result);
	});
}

function releaseAll(req, res){
	Profile.update({ name: req.params.name, locked: true }, { locked: false }, { multi: true }, function(err, results){
       res.json(results);
	});
}
